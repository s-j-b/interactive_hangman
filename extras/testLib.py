...
def matchLetter(letter, test):

    library = listdir("./UNCLETIKI")
    library.remove(".DS_Store")


    template = letter

    #structuring element in case we want to dilate the template or elements in the library
    #k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(19,19))
    
    #string list of all files in the UNCLETIKI directory, eg ['A1.png','A2.png',...]
    
    scores = []
    
    for element in library:
        #read the image
        image = cv2.imread('UNCLETIKI/'+element)
        image = cv2.split(image)[0]
        
        #match the template with that
        m = cv2.matchTemplate(image, template, cv2.TM_CCORR_NORMED)
        score = m[0][0]
        # Need some way to score the quality of the match, which we will append to the list of scores, a
        # from which we can pick the best score
        scores.append(score)

    #index to and display the image with the best template match score
    best = library[scores.index(max(scores))]
    best = cv2.imread('UNCLETIKI/'+best)

    cv2.imshow(test, template)
    cv2.waitKey()

    #display the 'best' match
    cv2.imshow(test, best)
    if cv2.waitKey() == ord('a'):
        name = "toSort/" + str(time())+".png"
